...
export const getTermsAndConditionsByID = async (userId: string): Promise<TermsAndConditionsType | null> => {
    try {
        const response = await axiosInstance.get(`/terms-and-conditions/${userId}`);
        return response.data;
    } catch (error) {
        handleError(error);
        return null;
